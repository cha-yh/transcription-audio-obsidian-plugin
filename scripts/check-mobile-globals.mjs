...
const FORBIDDEN = [
  ...
  { name: "setImmediate", re: /(?<![A-Za-z0-9_$.])setImmediate\s*\(/g },
  {
    ...
    re: new RegExp(
      `require\\s*\\(\\s*["'](?:node:[a-z_/]+|${NODE_BUILTINS})["']\\s*\\)`,
      "g"
    ),
  },
...
if (!existsSync(BUNDLE)) {
  console.error(
    `check-mobile-globals: ${BUNDLE} not found. Run the build first.`
  );
  process.exit(1);
}
...
if (hits.length === 0) {
  console.log(
    `check-mobile-globals: ${path.basename(
      BUNDLE
    )} is free of desktop-only globals ` +
...
console.error(
  `check-mobile-globals: ${
    hits.length
  } desktop-only global reference(s) in ${path.basename(BUNDLE)}.\n` +
